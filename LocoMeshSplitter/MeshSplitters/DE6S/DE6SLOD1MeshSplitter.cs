...
{
	internal class DE6SLOD1MeshSplitter
	{
		...
		{
			Mesh locoMesh = MeshFinder.FindMesh("de6_slug_LOD1");
			if (locoMesh is null)
			{
				Main.Logger.Critical($"MeshSplitter can't find the de6_slug LOD1 mesh");
				return null;
			}
			Main.Logger.Log($"Splitting diesel_body_LOD1 mesh...");

			GameObject splitLocoBodyLOD0 = new GameObject("de6_slug_LOD1");
			...
			Main.Logger.Log("Split de6_slug_LOD1 mesh.");
